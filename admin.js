// Handle create group with data
async function handleCreateGroupWithData(e) {
  e.preventDefault();
  const msg = document.getElementById('createGroupWithDataMessage');
  msg.textContent = '';

  const name = document.getElementById('cgDataName').value.trim();
  const description = document.getElementById('cgDataDesc').value.trim();
  const template = document.getElementById('cgDataTemplate').value;
  const logoUrl = document.getElementById('cgDataLogo').value.trim();
  ...
  msg.textContent = 'Creating group with sample data...';

  try {
    await adminApi('/api/groups/with-data', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, description, template, logoUrl })
    });
    msg.textContent = 'Group with sample data created.';
    document.getElementById('cgDataName').value = '';
    document.getElementById('cgDataDesc').value = '';
    document.getElementById('cgDataLogo').value = '';
    await loadGroups();
  } catch (err) {
    ...
    return `
      <tr data-group-id="${g._id}">
        ...
        <td>
          ...
          <button class="button button-danger" data-action="delete-group-hard" data-group-id="${g._id}" style="margin-left:8px;">Delete Group (Full Remove)</button>
        </td>
      </tr>
    `;
  }).join('');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // New: create group with data
  document.getElementById('createGroupWithDataForm').addEventListener('submit', handleCreateGroupWithData);
  ...
  wrap.addEventListener('click', async (e) => {
    ...
    } else if (action === 'delete-group-hard') {
      if (!confirm('Are you sure you want to permanently delete this group and ALL its data? This cannot be undone.')) return;
      try {
        await adminApi(`/api/groups/${encodeURIComponent(groupId)}/hard`, { method: 'DELETE' });
        alert('Group and all related data deleted.');
        await loadGroups();
      } catch (err) {
